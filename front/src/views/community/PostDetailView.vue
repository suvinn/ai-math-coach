<!-- 📄 src/views/community/PostDetailView.vue -->
<script setup>
import { ref, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import api, { unwrap } from '@/api'
import { useAuthStore } from '@/stores/auth'
import { useToast } from '@/composables/useToast'
import FocusShell from '@/components/common/FocusShell.vue'
import WdsButton from '@/components/common/WdsButton.vue'
import WdsIcon from '@/components/common/WdsIcon.vue'
import InlineTex from '@/components/common/InlineTex.vue'
import Toast from '@/components/common/Toast.vue'

const route  = useRoute()
const router = useRouter()
const auth   = useAuthStore()
const { toast, showToast } = useToast()

const problemId = route.params.problem_id
const postId    = route.params.post_id
const loading   = ref(true)
const post      = ref(null)
const newComment = ref('')
const submitting  = ref(false)

onMounted(async () => {
  try {
    post.value = unwrap(await api.get(`/problems/${problemId}/posts/${postId}`))
  } catch {
    showToast('게시글을 불러오지 못했어요', 'negative', 'circle-exclamation')
  } finally {
    loading.value = false
  }
})

async function submitComment() {
  if (!newComment.value.trim()) {
    showToast('댓글을 입력해주세요', 'negative', 'circle-exclamation')
    return
  }
  submitting.value = true
  try {
    const created = unwrap(
      await api.post(`/problems/${problemId}/posts/${postId}/comments`, {
        content: newComment.value.trim()
      })
    )
    post.value.comments.push(created)
    newComment.value = ''
  } catch (e) {
    const msg = e?.response?.data?.message || '댓글 작성에 실패했어요'
    showToast(msg, 'negative', 'circle-exclamation')
  } finally {
    submitting.value = false
  }
}

async function deleteComment(commentId) {
  if (!confirm('댓글을 삭제할까요?')) return
  try {
    await api.delete(`/problems/${problemId}/posts/${postId}/comments/${commentId}`)
    post.value.comments = post.value.comments.filter(c => c.id !== commentId)
    showToast('댓글이 삭제됐어요', 'positive', 'circle-check')
  } catch (e) {
    const msg = e?.response?.data?.message || '삭제에 실패했어요'
    showToast(msg, 'negative', 'circle-exclamation')
  }
}

async function deletePost() {
  if (!confirm('게시글을 삭제할까요?')) return
  try {
    await api.delete(`/problems/${problemId}/posts/${postId}`)
    showToast('게시글이 삭제됐어요', 'positive', 'circle-check')
    router.back()
  } catch (e) {
    const msg = e?.response?.data?.message || '삭제에 실패했어요'
    showToast(msg, 'negative', 'circle-exclamation')
  }
}

function formatDate(iso) {
  if (!iso) return ''
  return iso.slice(0, 10) + ' ' + iso.slice(11, 16)
}
</script>

<template>
  <FocusShell title="토론" :toast="toast" @back="router.back()">
    <Toast :toast="toast" />

    <div v-if="loading" class="center-msg assistive">불러오는 중…</div>

    <template v-else-if="post">
      <!-- 게시글 본문 -->
      <div class="post-box">
        <div class="post-header">
          <div class="post-avatar post-avatar--blue">
            <WdsIcon name="message" :size="20" color="#fff" />
          </div>
          <div class="post-header-info">
            <div class="post-title">{{ post.title }}</div>
            <div class="post-meta">
              <WdsIcon name="nav-mypage" :size="12" color="var(--label-assistive)" />
              <span class="meta-text">{{ post.name || post.username }}</span>
              <span class="meta-dot">·</span>
              <WdsIcon name="clock" :size="12" color="var(--label-assistive)" />
              <span class="meta-text">{{ formatDate(post.created_at) }}</span>
            </div>
          </div>
          <button
            v-if="auth.user?.username === post.username"
            class="delete-btn"
            @click="deletePost"
          >삭제</button>
        </div>
        <div class="post-divider" />
        <div class="post-content wds-body-2">
          <InlineTex :text="post.content" />
        </div>
      </div>

      <!-- 댓글 목록 -->
      <div class="comment-head">
        <span class="comment-head-bar" />
        <span class="wds-label-1" style="font-weight:700">댓글 <span style="color:var(--suql-accent)">{{ post.comments.length }}</span>개</span>
      </div>

      <div v-if="post.comments.length" class="stack-12">
        <div
          v-for="c in post.comments"
          :key="c.id"
          class="comment-card"
        >
          <div class="comment-author-row">
            <div class="comment-author-meta">
              <div class="comment-avatar">
                <WdsIcon name="nav-mypage" :size="18" color="var(--label-assistive)" />
              </div>
              <span class="comment-author">{{ c.name || c.username }}</span>
              <span class="meta-dot">·</span>
              <WdsIcon name="clock" :size="12" color="var(--label-assistive)" />
              <span class="wds-caption-1 assistive">{{ formatDate(c.created_at) }}</span>
            </div>
            <button
              v-if="auth.user?.username === c.username"
              class="delete-btn"
              @click="deleteComment(c.id)"
            >삭제</button>
          </div>
          <div class="comment-content wds-body-2">
            <InlineTex :text="c.content" />
          </div>
        </div>
      </div>
      <div v-else class="center-msg assistive">
        아직 댓글이 없어요. 첫 번째로 댓글을 달아보세요!
      </div>

      <!-- 댓글 작성 -->
      <div v-if="auth.isLoggedIn" class="write-box">
        <div class="comment-head" style="margin-bottom:8px">
          <span class="comment-head-bar" />
          <span class="wds-label-1" style="font-weight:700">댓글 쓰기</span>
        </div>
        <textarea
          v-model="newComment"
          class="comment-textarea"
          placeholder="댓글을 입력해주세요"
          rows="3"
          @input="e => { e.target.style.height = 'auto'; e.target.style.height = e.target.scrollHeight + 'px' }"
        />
        <div class="write-foot">
          <WdsButton
            variant="primary"
            size="medium"
            :disabled="submitting"
            @click="submitComment"
          >
            {{ submitting ? '등록 중…' : '댓글 등록' }}
          </WdsButton>
        </div>
      </div>
      <div v-else class="login-nudge wds-body-2">
        댓글을 작성하려면
        <button class="nudge-link" @click="router.push('/login')">로그인</button>
        이 필요해요.
      </div>
    </template>
  </FocusShell>
</template>

<style scoped>
.center-msg {
  display: flex; align-items: center; justify-content: center;
  padding: 60px 0; text-align: center;
}

/* 게시글 */
.post-box {
  padding: 16px 20px;
  border-radius: 16px;
  border: 1px solid var(--line-normal-normal);
  background: var(--background-normal-normal, #fff);
  box-shadow: 0 2px 8px rgba(0,0,0,0.06);
  display: flex;
  flex-direction: column;
  gap: 12px;
  margin-bottom: 24px;
}
.post-header {
  display: flex;
  align-items: flex-start;
  gap: 12px;
}
.post-avatar {
  width: 40px;
  height: 40px;
  border-radius: var(--radius-full);
  display: flex;
  align-items: center;
  justify-content: center;
  flex: none;
}
.post-avatar--blue {
  background: var(--suql-accent, #4B6FFF);
}
.post-header-info {
  flex: 1;
  display: flex;
  flex-direction: column;
  gap: 4px;
}
.post-title {
  font: var(--weight-bold) 16px/1.4 var(--font-sans);
  color: var(--label-normal);
}
.post-meta {
  display: flex;
  align-items: center;
  gap: 4px;
}
.meta-text {
  font: var(--weight-regular) 12px/1 var(--font-sans);
  color: var(--label-assistive);
}
.meta-dot {
  color: var(--label-assistive);
  font-size: 12px;
}
.post-divider {
  height: 1px;
  background: var(--line-normal-normal);
}
.post-content {
  color: var(--label-normal);
  line-height: 1.7;
  white-space: pre-wrap;
}

/* 댓글 헤더 */
.comment-head {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 12px;
}
.comment-head-bar {
  width: 3px;
  height: 16px;
  border-radius: 2px;
  background: var(--suql-accent, #4B6FFF);
  flex: none;
}

/* 댓글 카드 */
.comment-card {
  padding: 12px 16px;
  border-radius: 12px;
  border: 1px solid var(--line-normal-normal);
  background: var(--background-normal-normal, #fff);
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.comment-author-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.comment-author-meta {
  display: flex;
  align-items: center;
  gap: 4px;
}
.comment-avatar {
  width: 24px;
  height: 24px;
  border-radius: var(--radius-full);
  background: none;
  display: flex;
  align-items: center;
  justify-content: center;
  flex: none;
}
.comment-author {
  font: var(--weight-semibold) 13px/1 var(--font-sans);
  color: var(--label-normal);
}
.comment-content {
  color: var(--label-normal);
  line-height: 1.6;
  white-space: pre-wrap;
  padding-left: 4px;
}

/* 댓글 작성 */
.write-box {
  margin-top: 16px;
  display: flex;
  flex-direction: column;
  gap: 10px;
  padding: 16px;
  border-radius: 16px;
  border: 1px solid var(--line-normal-normal);
  background: var(--background-normal-normal, #fff);
}
.comment-textarea {
  width: 100%;
  box-sizing: border-box;
  padding: 12px 14px;
  border-radius: 10px;
  border: 1px solid var(--line-normal-normal);
  background: var(--fill-alternative);
  font: var(--weight-regular) 14px/1.6 var(--font-sans);
  color: var(--label-normal);
  resize: none;
  overflow: hidden;
  outline: none;
}
.comment-textarea:focus {
  border-color: var(--suql-accent);
  background: #fff;
}
.comment-textarea::placeholder {
  color: var(--label-assistive);
}
.write-foot {
  display: flex;
  justify-content: flex-end;
}
.delete-btn {
  background: none;
  border: none;
  font: var(--weight-medium) 12px/1 var(--font-sans);
  color: var(--label-assistive);
  cursor: pointer;
  padding: 2px 6px;
  border-radius: 6px;
  flex: none;
}
.delete-btn:hover { color: var(--status-negative); background: var(--red-99); }
.login-nudge {
  margin-top: 8px;
  text-align: center;
  color: var(--label-assistive);
}
.nudge-link {
  background: none;
  border: none;
  color: var(--suql-accent);
  font: inherit;
  font-weight: var(--weight-semibold);
  cursor: pointer;
  text-decoration: underline;
}
</style>